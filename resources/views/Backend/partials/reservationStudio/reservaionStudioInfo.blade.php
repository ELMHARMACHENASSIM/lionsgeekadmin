            ...
            @foreach ($allStudios as $studio)
                @if ($studio->id === $chosenStudio->id)
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href={{ route('reservationStudio.create', $studio) }}>
                            ...
                            var classCalendar = $("#reservationStudio").fullCalendar({
                                ...
                                customButtons: {
                                    myCustomButton: {
                                        ...
                                        click: function() {
                                            $.ajax({
                                                url: '/reservationStudio/downloadTxtAjax', // URL to the AJAX route
                                                method: 'GET',
                                                dataType: 'json', // Response type expected from the server
                                                success: function(data) {
                                                    // Handle the data received from the server (e.g., display it on the page).
                                                    var today = new Date().toDateString();
                                                    console.log(data.data);

                                                    // If you want to create a download link, you can do this:
                                                    const downloadLink = document.createElement('a');
                                                    downloadLink.href = 'data:text/plain;charset=utf-8,' +
                                                        encodeURIComponent(data.data);
                                                    downloadLink.download = today + '.csv';
                                                    downloadLink.click();
                                                },
