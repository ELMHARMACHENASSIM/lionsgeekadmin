...
@section('admin')
    <section class="content">
        <div class="containerme p-5">
            ...
            <div class="infouser">
                <a href={{ route('classes.index') }}><i class="fa-solid fa-chevron-left"></i></a>
                <h1 class="ms-2">Modifier la classe</h1>
            </div>
            ...
            <div class="d-flex align-items-start tabscontent">
                <div class="nav flex-column nav-pills me-5" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    <button class="nav-link text-noir btnclick active" id="v-pills-home-tab" data-bs-toggle="pill"
                        ...
                        aria-selected="true">Modifier la classe</button>
                ...
                <div class="tab-content maintabcontent" id="v-pills-tabContent">
                    <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel"
                        ...
                        <div class="updateclass">
                            <form action={{ route('class.update', $classe) }} method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="labelinput ">
                                    <label for="">Nom de classe</label>
                                    <input name="name" value="{{ old('name', $classe->name) }}" type="text"
                                        placeholder="Nom de classe" class="form-control mt-2 p-2" required>
                                </div>
                                <div class="labelinput">
                                    <label for="">Disponible</label>
                                    <select name="disponible" class="form-select mt-2" aria-label="Default select example">
                                        <option value="1" {{ $classe->disponible ? 'selected' : '' }}>oui</option>
                                        <option value="0" {{ !$classe->disponible ? 'selected' : '' }}>non</option>
                                    </select>
                                ...
                                <div class="d-flex align-items-center justify-content-center">
                                    ...
                                    <button type="submit" class="rounded">Modifier la classe</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="tab-pane fade imageupdate" id="v-pills-profile" role="tabpanel"
                        ...
                        <div class="updateclass">
                            <div class="headerupdate">
                                ...
                                <form action={{ route('classimage.store', $classe) }} method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="d-flex justify-content-evenly align-items-center ">
                                            <input name="images[]" class="form-control w-50  " type="file" id="formFile" multiple>
                            ...
                            <div class="classes-images">
                                @foreach ($classe->images as $image)
                                    <div class="images pb-2 ">
                                        <div class="imageclass">
                                            <img src={{ asset('storage/img/classe/' . $image->images) }} alt="">
                                        </div>
                                        ...
                                        <form class="formul" action={{ route('classimage.delete', $image) }}
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
